...
def run_unitary(H, tlist, options=None) -> qt.Qobj:
    """Compute the time-evolution operator U(T) via qt.propagator."""
    kwargs = {} if options is None else {"options": options}
...
def dqd_populations(
    ...
    """
    Extract DQD populations by partial trace over the photon subspace.

    basis='energy'      → (p_ground, p_excited1)
    basis='spin_charge' → np.ndarray of shape (16,)
    """
...
def vacuum_subspace_unitary(U: qt.Qobj, sys=None) -> np.ndarray:
    """Extract the zero-photon (vacuum) 16×16 block of the propagator U."""
    dqd_dim = 16
...
def two_qubit_unitary(U_full: qt.Qobj, sys: DQDsystem) -> np.ndarray:
    """Project onto the 4-level two-qubit subspace in the vacuum sector."""
